...
function normalizeCompletionFallbackTask(value, sourceDate) {
  if (!value || typeof value !== "object") return null;

  const text = String(value.text || "").trim();
  const sourceBatchKey = String(value.sourceBatchKey || "").trim();
  if (!text || !sourceBatchKey) return null;

  const sourceTaskOrder = Number.parseInt(value.sourceTaskOrder, 10);
  const days = Math.max(1, Number.parseInt(value.days, 10) || 1);
  return {
    item: String(value.item || "").trim(),
    text,
    units: Math.max(0, Number.parseFloat(value.units) || 0),
    scheduleDate: isIsoDate(value.scheduleDate) ? value.scheduleDate : sourceDate,
    totalHours: Math.max(0, Number.parseFloat(value.totalHours) || 0),
    days,
    assignments: Array.isArray(value.assignments) ? value.assignments : [],
    autoGenerated: true,
    sourceBatchKey,
    sourceBatchLabel: String(value.sourceBatchLabel || "").trim(),
    sourceTaskOrder: Number.isFinite(sourceTaskOrder) ? sourceTaskOrder : null,
    hoursOverridden: Boolean(value.hoursOverridden),
    generatedTaskPlanned: Boolean(value.generatedTaskPlanned),
    completedDates: Array.isArray(value.completedDates)
      ? value.completedDates.filter(isIsoDate)
      : []
  };
}
...
app.put("/schedule/task-completion", async (req, res) => {
  ...
  const fallbackTask = taskType === "processingTasks"
    ? normalizeCompletionFallbackTask(req.body.task, sourceDate)
    : null;
