...
test('editing an imported item clears its needs-date flag only after a valid save', async () => {
  ...
  database.exec(`CREATE TABLE ordered_items (id INTEGER PRIMARY KEY, date_ordered TEXT,
    ...
    department TEXT, received_by TEXT, updated_at TEXT);
  ...
  assert.equal(save({...payload, received_by:'Jennifer'}), 200);
  assert.equal(database.prepare('SELECT received_by FROM ordered_items WHERE id=1').get().received_by,'Jennifer');
