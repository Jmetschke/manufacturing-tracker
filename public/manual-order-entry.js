const ManualOrderEntry = (() => {
  function open() {
    ...
    const inputs = {};
    [['date_ordered','Date ordered','date'],['expected_delivery_date','Expected delivery date','date'],['item_name','Item description','text'],['package_qty','Package quantity','number'],['item_supplier','Retailer / supplier','text'],['department','Department','text']].forEach(([key,label,type]) => {
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        payload.package_qty = Number(payload.package_qty);
        ...
        saved = true;
        form.reset(); inputs.date_ordered.value = toIsoDate(new Date()); modal.hidden = true;
      ...
      finally { save.disabled = false; close.disabled = false; save.textContent = 'Add ordered item'; }
    });
    panel.append(heading, form); modal.append(panel); document.body.append(modal); inputs.item_name.focus();
  }
  return { open };
})();
