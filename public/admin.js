...
function getBatchColorKey(task) {
  ...
  const key = getGeneratedSourceBatchBaseKey(task.sourceBatchKey);
  return key;
}
...
function getGeneratedTaskCompletionKeys(task) {
  if (!task || !task.sourceBatchKey || !task.text) return [];
  const baseKey = getGeneratedSourceBatchBaseKey(task.sourceBatchKey);
  const order = task.sourceTaskOrder === null || task.sourceTaskOrder === undefined ? "" : String(task.sourceTaskOrder);
  const text = normalizeGeneratedTaskText(task.text);
  const label = String(task.sourceBatchLabel || "").trim();

  return [
    `base:${baseKey}:order:${order}:text:${text}`,
    label ? `label:${label}:order:${order}:text:${text}` : ""
  ].filter(Boolean);
}

function getCompletedGeneratedTaskKeys(tasks) {
  return tasks.reduce((keys, task) => {
    if (taskWasCompleted(task)) {
      getGeneratedTaskCompletionKeys(task).forEach(key => keys.add(key));
    }
    return keys;
  }, new Set());
}

function generatedTaskWasCompleted(task, completedGeneratedTaskKeys) {
  if (taskWasCompleted(task)) return true;
  return getGeneratedTaskCompletionKeys(task).some(key => completedGeneratedTaskKeys.has(key));
}
...
function buildAdminProjectedTasksByDate(rows, visibleStart, visibleEnd, taskSelector = payload => payload.tasks) {
  ...
  rows.forEach(row => {
    ...
    const tasksForProjection = taskType === "processingTasks"
      ? getProcessingTasksWithGeneratedBatchFallback(payload, row)
      : selectedTasks;
    const completedGeneratedTaskKeys = taskType === "processingTasks"
      ? getCompletedGeneratedTaskKeys(tasksForProjection)
      : new Set();

    tasksForProjection.forEach((task, taskIndex) => {
      if (taskType === "processingTasks" && task.autoGenerated && task.sourceBatchKey) {
        ...
        for (
          ...
          tasksByDate.get(activeDateIso).push(createProjectedTask(task, row, taskType, taskIndex, activeDateIso, 0, {
            ...
            remainingHours: completedForBatch ? 0 : task.totalHours
          }));
        }
        return;
      }
...
function buildAdminActiveScheduleByDate(rows, visibleStart, visibleEnd) {
  ...
  rows.forEach(row => {
    ...
    [
      ...
      const completedGeneratedTaskKeys = taskType === "processingTasks"
        ? getCompletedGeneratedTaskKeys(tasks)
        : new Set();
...
function getGeneratedTaskIdentityKey(task) {
  const baseKey = getGeneratedSourceBatchBaseKey(task.sourceBatchKey) || getBatchColorKey(task);
...
function groupDailyTaskAssignments(tasks) {
  ...
  tasks.forEach(task => {
    const generatedCompletionKeys = getGeneratedTaskCompletionKeys(task);
    const generatedTaskKey = task.taskType === "processingTasks" && generatedCompletionKeys.length
      ? `${task.taskType}\n${task.sourceScheduleDate || ""}\n${generatedCompletionKeys.find(key => key.startsWith("label:")) || generatedCompletionKeys[0]}`
      : "";
...
window.productionTrackerLaunchReady.then(initAdmin);
